...
class CustomBackend(ModelBackend):
    ...
    def authenticate(self, request, username=None, password=None, session_id=None, sig=None, token=None, scene=None, ip='127.0.0.1', **kwargs):# 重写这个函数
        import json
        ...
        if session_id:
            ...
            if json.loads(str(result, encoding="utf-8"))['Code'] == 900:
                raise serializers.ValidationError({'detail': '滑动验证有通过，请刷新重试！'})
        ...
        #     raise serializers.ValidationError({'detail': '滑动验证很通过，请刷新重试！'})
