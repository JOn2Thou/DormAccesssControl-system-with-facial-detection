...
from system_setting.models import SystemLog,tempUserInfo
...
class UserViewset(mixins.RetrieveModelMixin, mixins.ListModelMixin, mixins.UpdateModelMixin, mixins.CreateModelMixin, viewsets.GenericViewSet):
    ...
    def get_serializer_class(self):
        ...
        if self.action == "ttemp":
            return UserTempSerializer
    ...
    def ttemp(self, request, *args, **kwargs):
        """
            管理员创建单个用户
            url: '/users/'
            type: 'post'
        ...
        TempUserInfo = tempUserInfo.objects.create(mobile=serializer.validated_data["mobile"],
        ...
        system_log = SystemLog.objects.create(
            content='创建临时用户（用户名：' + serializer.validated_data["mobile"] + '）',
            category="用户管理",
            operator=request.user,
            ip=request.META.get("REMOTE_ADDR"))
        system_log.save()
...
class QQLoginViewset(viewsets.GenericViewSet):
    ...
    def login(self, request, *args, **kwargs):
        ...
        if code == '' or state == '':
            return redirect('http://localhost:8080/views/qqlogin.html?error_code=99999')
        if state == 'bind':
            return redirect('http://localhost:8080/views/qqlogin.html?code='+code)
        ...
        if 'callback' in token_result.text:
            ...
            return redirect('http://localhost:8080/views/qqlogin.html?error_code=' + token_result_json['error'])
        else:
            token_result_json = {}
        ...
        if me_result_json['client_id'] != settings.QQCONNECT_ID:
            return redirect('http://localhost:8080/views/qqlogin.html?error_code=99998')
        ...
        if state == 'login':
            if QQLoginTokenModel.objects.filter(openid=me_result_json['openid']):
                ...
                return redirect('http://localhost:8080/views/qqlogin.html?token=' + token)
            else:
                return redirect('http://localhost:8080/views/qqlogin.html?error_code=99997')
