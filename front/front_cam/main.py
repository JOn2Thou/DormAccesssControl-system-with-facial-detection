...
mysql_post = 3306
mysql_user = "root"
mysql_password = "Db259915534"
mysql_db = "attendance_system"
login_url = "http://localhost:8080/login/"
login_header = {'Content-Type': 'application/json'}
login_username = "wuchu"
login_password = "wuchu"
upload_url = "http://localhost:8080/access_control/"
...
cap = cv2.VideoCapture('http://admin:admin@10.252.40.204:8081')
# 设置视频参数 set camera
cap.set(3, 480)
...
def upload_image(save_name, user_id, euclidean_distance):
    ...
    url = upload_url + '?user_id=' + str(user_id) + '&euclidean_distance=' + str(euclidean_distance)
    print("url:",url)
    upload_post_result = requests.post(url=upload_url + '?user_id=' + str(user_id) + '&euclidean_distance=' + str(euclidean_distance),
                                       ...
                                       files=file)
    print("upload_post_result:", upload_post_result.status_code)
    # print("[BACKEND]", json.loads(upload_post_result.text)['detail'])
    try:
        response_data = json.loads(upload_post_result.text)
        detail = response_data['detail']
        print("[BACKEND]", detail)
    except (json.JSONDecodeError, KeyError) as e:
        print("[BACKEND] Error parsing response:", str(e))
